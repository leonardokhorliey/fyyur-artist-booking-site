...
def search_artists():
  ...
  for result in search_result:
    artist_upcoming_shows = db.session.query(Show).filter(Show.artist_id == result.id).filter(Show.start_time > datetime.now()).all()
...
def edit_artist_submission(artist_id):
  ...
  try: 
    ...
    if is_successful:
      ...
      flash("Update could not be done. Encountered an error")
...
def create_show_submission():
  ...
  try: 
    ...
    if is_successful:
      ...
      flash("Error encountered! Show could not be listed")
  return render_template('pages/home.html')
